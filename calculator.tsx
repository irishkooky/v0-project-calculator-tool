...
export default function PriceCalculator() {
  ...
  // 売上額入力フィールドの追加
  const addSaleAmount = () => {
    setSaleAmounts([...saleAmounts, { id: Date.now().toString(), amount: 0 }])
  }
              ...
              <div key={item.id} className="flex items-center gap-2">
                <div className="relative flex-1">
                  <Input
                    ...
                    placeholder="例：100"
                    value={item.amount || ""}
                    onChange={(e) => updateSaleAmount(item.id, Number(e.target.value) || 0)}
                    className="pr-12"
                  />
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="relative w-32">
                <Input
                  type="number"
                  value={bFixedAmountInMan}
                  onChange={(e) => setBFixedAmountInMan(Number(e.target.value) || 0)}
                  className="pr-12"
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500">万円</span>
              </div>
